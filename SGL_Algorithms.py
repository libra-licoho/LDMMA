...
def MMCauchy(data_info,MM_Setting = dict()):
    ...
    r = MM_Setting["initial_guess_r"] if "initial_guess" in MM_Setting.keys() else .1*np.ones(M+1)
    lbd = MM_Setting["initial_guess_lbd"] if "initial_guess" in MM_Setting.keys() else 5*np.ones(M+1)
    x = np.zeros(settings.num_features)
    ...
    class MM_approximated:
        def __init__(self, settings, data, MM_Setting = dict()):
            ...
            self.constraints = [cp.pnorm(self.x[group_ind[i]:group_ind[i+1]], 2) <= self.r[i] for i in range(M)] + [cp.pnorm(self.x, 1) <= self.r[M]]
            self.constraints += [cp.pnorm(self.rho1[group_ind[i]:group_ind[i+1]], 2) <= self.lbd[i] for i in range(M)] + [cp.pnorm(self.rho2, np.inf) <= self.lbd[M]]
